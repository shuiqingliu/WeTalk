...
- (void)moreKeyboard:(id)keyboard didSelectedFunctionItem:(TLMoreKeyboardItem *)funcItem
{
    ...
    }else if (funcItem.type == TLMoreKeyboardItemTypeFile){
        FileViewController *fileController = [[FileViewController alloc] init];
        fileController.chatName = @"选择文件";
        //hide bottom bar
...
- (void)sendFile:(NSString *)fileName{
    __weak typeof(self) weakSelf = self;
    [weakSelf sendFileMessage:fileName];;
}
